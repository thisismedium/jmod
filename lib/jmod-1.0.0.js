...
(function() {
    ...
    // Mechanism for requesting modules for download, usually over HTTP
    // unless a namespace has been customized
    function request_modules() {
        var names = arguments_to_array(arguments);
        var rest = namespace_dispatch(names);
    ...
    function namespace_dispatch(names) {
        var unprocessed = [];
    ...
    function request(name, url) {
        if(!is_requested(name)) {
            ...
            onReady(function() {
                get_script(url || resolve_url(name));
            });
            REQUESTED[name] = true;
        }
    }
