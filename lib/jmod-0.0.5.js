...
(function() {
    ...
    function make_module(name, imps, code) {
        obj = { name: name,
                imports: imps,
                code: code };
        // code is a javascript function which represents
        // the module.  By linking it with the module instance,
        // we can retrieve it during executing, which
        // is useful for debugging and introspection purposes.
        obj.code.module = obj;
        return obj;
    }
    ...
    function set_namespace_handler(name, path_transformer) {
        NAMESPACES[name] = path_transformer;
    }

    function set_namespace_path(name, path) {
        NAMESPACES[name] = path;
    }
    
    function request_namespaces(names) {
        var unprocessed = [];
        each(group_by_prefix(filter_requested(names)),
             function(prefix, names) {
                 if((handler = NAMESPACES[prefix])) {
                     if(typeof(handler) == "string") {
                         each(names, function(i, name) {
                             // The given path essentially replaces the
                             // top-level namespace, so strip it off,
                             // while still registering it under its
                             // full name
                             request(name,
                                     handler,
                                     name_to_path(strip_prefix(name)));
                         });
                     }
                     else {
                         handler(names);
    ...
    function group_by_prefix(names) {
        var grouped = {};
        each(names, function(i, name) {
    ...
    function filter_requested(names) {
        var rest = [];
        each(names, function(i, name) {
    ...
    function load(module) {
        ...
        // Don't put the module in the global namespace
        // since it conflicts if libraries set properties
        // of the same name.  You should always import libraries
        // and use the renaming mechanism to access it.
        // - jwl 3/25/2009
    ...
    window.jMod = {
        ...
        ready: onReady,
        set_namespace_handler: set_namespace_handler,
        set_namespace_path: set_namespace_path,
        set_global_path: function(path) {
            JS_URL = path;
        }
    };
